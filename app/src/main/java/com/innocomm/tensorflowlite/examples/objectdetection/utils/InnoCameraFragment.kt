 ...
 */
abstract class InnoCameraFragment : BaseFragment(), ICameraStateCallBack {
        ...
        mCameraClient = MultiCameraClient(requireContext(), object : IDeviceConnectCallBack {
            ...
            override fun onConnectDev(device: UsbDevice?, ctrlBlock: USBMonitor.UsbControlBlock?) {
                ...
                mCameraMap[device.deviceId]?.apply {
                    ...
                    //innocomm
                    mCurrentCamera?.let { addPreviewDataCallBack(object: IPreviewDataCallBack{
...
//innocomm
    ...
    protected fun updateResolution(width: Int, height: Int) {
        mCameraView?.let {
            it.setAspectRatio(width,height)
        }
    ...
    protected fun openCamera(st: IAspectRatio? = null) {
        when (st) {
            ...
            getCurrentCamera()?.setCameraStateCallBack(this@InnoCameraFragment)
        }
    }
    ...
    private fun surfaceSizeChanged(surfaceWidth: Int, surfaceHeight: Int) {
        Log.v("innocomm", "surfaceSizeChanged:"+surfaceWidth+"x"+surfaceHeight)
        onSurfaceSizeChanged(surfaceWidth,surfaceHeight)
        getCurrentCamera()?.setRenderSize(surfaceWidth, surfaceHeight)
    }

    abstract fun onSurfaceSizeChanged(surfaceWidth: Int, surfaceHeight: Int)
    ...
    protected open fun getCameraRequest(): CameraRequest {
        ...
        return CameraRequest.Builder()
            .setPreviewWidth(Application.previewSize.width)
            .setPreviewHeight(Application.previewSize.height)
            .setRenderMode(CameraRequest.RenderMode.NORMAL)
            .setDefaultRotateType(RotateType.ANGLE_0)
            .setAudioSource(CameraRequest.AudioSource.NONE)//innocomm CameraRequest.AudioSource.SOURCE_SYS_MIC)
